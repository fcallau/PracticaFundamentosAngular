...
export class ProductService {
  ...
  getProducts(filter: ProductFilter = undefined): Observable<Product[]> {
    ...
    if (filter) {
      if (filter.text) {
        options.search.set('q', filter.text);
      }

      if (filter.category && filter.category !== '0') {
        options.search.set('category.id', filter.category);
      }
    }
    ...
    if (filter) {
      if (filter.state === 'sold') {
        options.search.set('state', 'sold');
      }
    }

    /*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~|
    | Red Wine Path (sin Pipes)                                        |
    |~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/

    /*if (filter) {
      if (filter.state && filter.state !== '0') {
        if (filter.state === '1') {
          options.search.set('state', 'selling');
        } else {
          options.search.set('state', 'sold');
        }
      }
    ...
    if (filter) {
      if (filter.minPrecio) {
        options.search.set('price_gte', filter.minPrecio);
      }

      if (filter.maxPrecio) {
        options.search.set('price_lte', filter.maxPrecio);
      }

      // Se conserva el criterio de ordenación del Pink Path. Se añade la ordenación por precio o alfabética si se selecciona el criterio
      if (filter.sortField === '1') {
        options.search.set('_sort', 'price,publishedDate');
        options.search.set('_order', 'ASC,DESC');
      }

      if (filter.sortField === '2') {
        options.search.set('_sort', 'name,publishedDate');
        options.search.set('_order', 'ASC,DESC');
      }
    }*/
